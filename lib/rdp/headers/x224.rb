...
#
#
# All the Transport Protocol Data Units (TPDUs) shall contain an integral number of bytes. The bytes in a TPDU are
# numbered starting from 1 and increasing in the order they are put into an NDSU. The bits in a byte are numbered
...
# NOTES
# 1 The numbering of bits within a byte is a convention local to this Recommendation | International Standard.
...
# 4 As described in 6.2.3, both transport entities respect these bit and byte ordering conventions, thus allowing
#   communication to take place.
# 5 In this clause the encoding of TPDUs is represented in the following form:
#    a) bytes are shown with the lowest numbered byte to the left; higher numbered bytes being further to the right;
#    b) within a byte, bits are shown with bit 8 to the left and bit 1 to the right.
#
...
# The field is contained in the first byte of the TPDUs. The length is indicated by a binary number, with a maximum value
# of 254 (1111 1110). The length indicated shall be the header length in bytes including parameters, but excluding the
...
#
# 13.2.2.2 TPDU code
# This field contains the TPDU code and is contained in byte 2 of the header. It is used to define the structure of the
# remaining header. This field is a full byte except in the following cases:
...
module RDP
  ...
  class X224Header
    ...
    def parse!
      ...
      @length, @pdu_type      = full_data.unpack('C*')
      @var_length             = 0
      @neg_req_length         = 0
    ...
    def extract_variable
      ...
      if @length >= RDP::RDP_COOKIE_MIN_SIZE
        end_of_variable_part = @payload.index("\r\n")+1
        @variable_part = @payload.byteslice(RDP::X224_CRQ_SIZE..end_of_variable_part)
